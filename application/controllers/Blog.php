...
class Blog extends CI_Controller{
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'blog/index/';
			...
			$x['populer']=$this->db->query("SELECT * FROM posts ORDER BY id DESC LIMIT 5");
			$this->load->view('fe/v_blog',$x);
	}
	...
	function detail($slugs){
		...
		if($query->num_rows() > 0){
			...
			$x['title']=$row['title'];
			$x['blog'] =$row['content'];
			$x['tanggal']=$row['created_date'];
			$x['kategori']=$row['category'];
			$x['slug']=$row['title'];
			$x['populer']=$this->db->query("SELECT * FROM posts ORDER BY id DESC LIMIT 5");
			$this->load->view('fe/v_blog_detail',$x);
		}else{
			redirect('fe/blog');
		}
	}
